...
const newEventSchema = z.object({
  ...
  eventLocation: z.string().min(1, "Event location is required").max(100, "Event location name is too long"),
  numTopTables: z.number().min(0, "Negative number inputs not valid").max(1, "You cannot have more than one top table"),
...
.refine(data => {
  ...
  return true;
}, { message: "You have entered a top table size with no top table. Please set table size to zero or add a top table", path: ['numTopTables', 'maxSizeTopTable'] });
...
const newEventForm = (
  <Stack spacing={6}>
    <FormControl>
      {fieldErrors.eventName && <div style={{ color: "red" }}>{fieldErrors.eventName}</div>}
      <FormLabel mb={1}>Event Name</FormLabel>
      <Input value={eventName} onChange={(e) => setEventName(e.target.value)} />
    </FormControl>
    <FormControl>
      {fieldErrors.eventDate && <div style={{ color: "red" }}>{fieldErrors.eventDate}</div>}
      <FormLabel mb={1}>Event Date</FormLabel>
      <Input type="date" value={eventDate} onChange={(e) => setEventDate(e.target.value)} />
    </FormControl>
    <FormControl>
      {fieldErrors.eventLocation && <div style={{ color: "red" }}>{fieldErrors.eventLocation}</div>}
      <FormLabel mb={1}>Event Location</FormLabel>
      <Input value={eventLocation} onChange={(e) => setEventLocation(e.target.value)} />
    </FormControl>
    <FormControl>
      {fieldErrors.numTopTables && <div style={{ color: "red" }}>{fieldErrors.numTopTables}</div>}
    ...
    <FormControl>
      {fieldErrors.maxSizeTopTable && <div style={{ color: "red" }}>{fieldErrors.maxSizeTopTable}</div>}
    ...
    <FormControl>
      {fieldErrors.numNormalTables && <div style={{ color: "red" }}>{fieldErrors.numNormalTables}</div>}
    ...
    <FormControl>
      {fieldErrors.maxSizeNormalTable && <div style={{ color: "red" }}>{fieldErrors.maxSizeNormalTable}</div>}
